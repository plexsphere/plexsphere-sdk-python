...
Single directed edge in the per-Domain mesh-topology projection. Mirrors a SSE peer-delta row one-for-one — a renderer that consumes the live stream and a renderer that polls `GET /v1/domains/{domain_id}/mesh/topology` converge on byte-identical state. 
...
**status** | **str** | Derived edge health, computed by applying the per-Domain reachability policy thresholds to the per-edge handshake observation. The handshake telemetry itself is deliberately NOT exposed on this envelope; a follow-up extends the &#x60;MeshTopology&#x60; envelope with the per-edge handshake observation. Uses the same enum values as the per-Node &#x60;Reachability&#x60; projection so a single renderer can colour both surfaces from one mapping.  | 
