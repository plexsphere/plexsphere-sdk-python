...
is gated by the `manage` relation on the platform singleton before any
data is returned.
...
step succeeded. The read is gated by the `manage` relation on the
platform singleton before any data is returned.
