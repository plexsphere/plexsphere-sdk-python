...
**external_ref** | **str** | Optional external-system reference for the adopted flow (URN, ARN, vendor-opaque id). Trimmed; whitespace-only is rejected. Rejected with &#x60;400 invalid_resource&#x60; on the provisioned flow: the platform assigns that Resource&#39;s handle itself, because the handle is what the enrolling agent presents at registration and it learns it from the material the provisioning broker renders, not from an operator.  | [optional] 
