...
Body for POST /v1/nodes/{id}/heartbeat success responses. Carries the server-side commit timestamp and two reconciliation hints. `reconcile` defaults to `false` until later stories wire the divergence detector; `rotate_keys` is load-bearing today and dispatches a Curve25519 mesh-key rotation — it never rotates the NSK. 
