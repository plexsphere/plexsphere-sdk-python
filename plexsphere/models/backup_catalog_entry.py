...
class BackupCatalogEntry(BaseModel):
    ...
    """ # noqa: E501
    store: StrictStr = Field(description="The store the row describes.")
    method: StrictStr = Field(description="How the store is backed up.")
    cadence: StrictStr = Field(description="How often the store is backed up.")
    retention: StrictStr = Field(description="How long the store's backups are retained.")
    priority: BackupPriority
    restore_step: Annotated[int, Field(strict=True, ge=0)] = Field(description="The restore-plan step order this store is restored in, or `0` when the store is not part of the linear restore sequence. ")
    notes: StrictStr = Field(description="Free-form notes about backing up or restoring the store.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["store", "method", "cadence", "retention", "priority", "restore_step", "notes"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "restore_step": obj.get("restore_step"),
            "notes": obj.get("notes")
        })
