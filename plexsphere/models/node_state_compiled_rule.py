...
class NodeStateCompiledRule(BaseModel):
    ...
    source_cidr: Annotated[str, Field(strict=True, max_length=49)] = Field(description="IPv4 or IPv6 prefix in CIDR notation (e.g. `10.0.0.0/8`, `2001:db8::/32`). Source and destination agree on address family by aggregate-boundary invariant. ")
    destination_cidr: Annotated[str, Field(strict=True, max_length=49)] = Field(description="IPv4 or IPv6 prefix in CIDR notation. Address family matches `source_cidr`. ")
    ...
    def source_cidr_validate_regular_expression(cls, value):
    ...
    def destination_cidr_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not isinstance(value, str):
            value = str(value)
