...
class LookupResourcesRequest(BaseModel):
    ...
    """ # noqa: E501
    subject: Annotated[str, Field(min_length=1, strict=True, max_length=4096)] = Field(description="Object reference of the subject whose reachable resources are enumerated (e.g. `user:0190a8b8-...`, `service-identity:...`). ")
    relation: Annotated[str, Field(min_length=1, strict=True, max_length=4096)] = Field(description="Relation name to evaluate (e.g. `read`, `manage`). The accepted set is fixed by the schema in `schema/authz.zed`. ")
    resource_type: Annotated[str, Field(min_length=1, strict=True, max_length=4096)] = Field(description="Object type whose instances are enumerated (e.g. `project`, `domain`). Returned `items` are `<resource_type>:<id>` object references. ")
    caveat_context: Optional[Dict[str, Any]] = Field(default=None, description="Optional CEL caveat evaluation context — a map from caveat field NAME to VALUE — forwarded verbatim to the authorizer so a caveated relation can be evaluated at check time. Values DO cross this boundary (they are the evaluation inputs); it is the AUDIT projection of this call that records field NAMES only, never the values. ")
