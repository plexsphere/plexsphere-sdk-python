...
class CredentialAssignmentResponse(BaseModel):
    ...
    cloud_credential_id: UUID = Field(description="Identifier of the Cloud Credential bound to the Project by this assignment. ")
    state: CredentialAssignmentState
    materialised: StrictBool = Field(description="Whether the assignment's binding is currently live. `true` only while the assignment is in the `approved` state; `false` for `requested`, `rejected`, and `revoked`. ")
    ...
    __properties: ClassVar[List[str]] = ["id", "project_id", "cloud_credential_id", "state", "materialised", "created_at", "updated_at"]
