...
class LabelValueSchema(BaseModel):
    ...
    values: Optional[Annotated[List[StrictStr], Field(max_length=256)]] = Field(default=None, description="Permitted string values for `kind=enum`. Non-empty, unique, non-blank. ")
    min: Optional[StrictInt] = Field(default=None, description="Inclusive lower bound for `kind=numeric`. Omit for an open lower bound. ")
    max: Optional[StrictInt] = Field(default=None, description="Inclusive upper bound for `kind=numeric`. Omit for an open upper bound. ")
    pattern: Optional[StrictStr] = Field(default=None, description="Go regexp pattern for `kind=regex`. Compiled at aggregate build time; malformed patterns are rejected. ")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["kind", "max_len", "values", "min", "max", "pattern"]
