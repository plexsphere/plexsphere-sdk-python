...
class SSEEventSessionSetup(BaseModel):
    """
    ...
    """ # noqa: E501
    session_id: UUID = Field(description="Identifier of the Session being set up (UUIDv7).")
    jti: StrictStr = Field(description="The issued token's `jti`. Equals the session identifier. ")
    kind: SessionKind
    target: SessionTarget
    expires_at: datetime = Field(description="Expiry timestamp (UTC) the listener tears down at.")
    idle_timeout_seconds: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Idle window in whole seconds the listener applies locally. ")
    callback_url: StrictStr = Field(description="The per-session callback URL under `/v1/nodes/{id}/sessions/{session_id}` the Node posts per-kind activity to. ")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["session_id", "jti", "kind", "target", "expires_at", "idle_timeout_seconds", "callback_url"]
