...
class SessionTarget(BaseModel):
    ...
    """ # noqa: E501
    ssh: Optional[SessionTargetSSH] = Field(default=None, description="Populated only when `kind` is `ssh`. ")
    k8s: Optional[SessionTargetK8s] = Field(default=None, description="Populated only when `kind` is `k8s`. ")
    tcp: Optional[SessionTargetTCP] = Field(default=None, description="Populated only when `kind` is `tcp`. ")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "ssh": SessionTargetSSH.from_dict(obj["ssh"]) if obj.get("ssh") is not None else None,
            "k8s": SessionTargetK8s.from_dict(obj["k8s"]) if obj.get("k8s") is not None else None,
            "tcp": SessionTargetTCP.from_dict(obj["tcp"]) if obj.get("tcp") is not None else None
        })
