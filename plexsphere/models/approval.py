...
class Approval(BaseModel):
    ...
    decided_at: Optional[datetime] = Field(default=None, description="Timestamp the proposal reached a terminal state (UTC). `null` or omitted while the proposal is still `proposed` or `pending-approval`. ")
    decided_by_subject: Optional[StrictStr] = Field(default=None, description="ReBAC subject string of the principal that decided the proposal. `null` or omitted while undecided and for the unattended `expired` path. ")
    decision_reason: Optional[StrictStr] = Field(default=None, description="Free-text rationale recorded with the decision. `null` or omitted while undecided and for the unattended `expired` path. For a break-glass override the rationale value is PII and is NOT surfaced here verbatim — only its field name is projected onto `caveat_context`. ")
