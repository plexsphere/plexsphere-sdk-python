...
class SessionActivityRequest(BaseModel):
    ...
    """ # noqa: E501
    ssh: Optional[SessionActivitySSH] = Field(default=None, description="Populated only for an `ssh` session.")
    k8s: Optional[SessionActivityK8s] = Field(default=None, description="Populated only for a `k8s` session.")
    tcp: Optional[SessionActivityTCP] = Field(default=None, description="Populated only for a `tcp` session.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["ssh", "k8s", "tcp"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "ssh": SessionActivitySSH.from_dict(obj["ssh"]) if obj.get("ssh") is not None else None,
            "k8s": SessionActivityK8s.from_dict(obj["k8s"]) if obj.get("k8s") is not None else None,
            "tcp": SessionActivityTCP.from_dict(obj["tcp"]) if obj.get("tcp") is not None else None
        })
