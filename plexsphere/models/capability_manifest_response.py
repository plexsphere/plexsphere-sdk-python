...
class CapabilityManifestResponse(BaseModel):
    ...
    """ # noqa: E501
    accepted_at: datetime = Field(description="Server-side timestamp at which the capability manifest snapshot was committed. ")
    fields_changed: Annotated[List[StrictStr], Field(max_length=256)] = Field(description="Alphabetically-sorted list of manifest fields that transitioned versus the prior persisted snapshot. Possible entries: `binary_version`, `binary_checksum`, `ssh_host_key_fingerprint`, `declared_hooks`, `plexd_hooks`. Empty on an idempotent no-op PUT. ")
    host_key_changed: StrictBool = Field(description="`true` when the SSH host-key fingerprint transitioned versus the prior persisted snapshot (including a transition into or out of the unset state). Always present so downstream security correlators can branch on the flag without parsing `fields_changed`. ")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["accepted_at", "fields_changed", "host_key_changed"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "fields_changed": obj.get("fields_changed"),
            "host_key_changed": obj.get("host_key_changed")
        })
