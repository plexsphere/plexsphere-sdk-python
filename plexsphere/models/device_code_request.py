...
class DeviceCodeRequest(BaseModel):
    ...
    """ # noqa: E501
    domain_id: Optional[UUID] = Field(default=None, description="Domain the caller is authenticating against. Optional: omitting it together with a platform-shared binding named via `idp_binding_id` or `idp_binding_alias` starts a Domain-independent platform-operator device authorization. A per-Domain binding named without a `domain_id` is rejected 400 on this surface. ")
    ...
    idp_binding_alias: Optional[Annotated[str, Field(strict=True, max_length=4096)]] = Field(default=None, description="Human-friendly alias of an IdP binding within the Domain (e.g. `github`). Optional. Resolution precedence is explicit id, then alias, then the Domain's primary binding, then its single active binding. Mutually exclusive with idp_binding_id. With no `domain_id`, must name a platform-shared binding's alias (globally unique among active platform bindings) to start a platform-operator device authorization. ")
    client_id: Optional[Annotated[str, Field(strict=True, max_length=4096)]] = Field(default=None, description="Optional OIDC client identifier override.")
