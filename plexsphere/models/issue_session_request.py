...
class IssueSessionRequest(BaseModel):
    ...
    """ # noqa: E501
    resource_id: UUID = Field(description="Identifier of the Resource to open a session against.")
    kind: SessionKind
    target: SessionTarget
    requested_ttl_seconds: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, description="Operator-requested TTL in whole seconds. Omitted falls back to the per-Domain default; a value above the per-Domain maximum is silently clamped, not rejected — the response `expires_at` reflects the effective TTL. ")
    idempotency_key: Optional[Annotated[str, Field(strict=True, max_length=4096)]] = Field(default=None, description="Optional idempotency key. A repeat issuance with the same `(identity, idempotency_key)` within the dedupe window returns the original Session and token rather than minting a new one. ")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["resource_id", "kind", "target", "requested_ttl_seconds", "idempotency_key"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "requested_ttl_seconds": obj.get("requested_ttl_seconds"),
